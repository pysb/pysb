...
class Simulator(object):
    ...
    netgen : str, optional (default: 'auto')
        Network generator to use for expanding rules into species and
        reactions before simulation.  Allowed values:

        ``'auto'`` (default)
            If the model already has equations (``model.reactions`` is
            non-empty), use them as-is regardless of how they were
            generated.  Otherwise use ``'pysb'`` if the model was
            constructed with ``auto_netgen=True``, or ``'bng'`` if not.
        ``'bng'``
            Use BioNetGen via :func:`pysb.bng.generate_equations`.
            Requires a working BioNetGen installation.  Raises
            ``ValueError`` if the model has ``auto_netgen=True``; call
            ``model.reset_equations()`` and set ``model.auto_netgen =
            False`` first if you really want BNG for such a model.
        ``'pysb'``
            Use the native Python network generator
            :class:`pysb.netgen.NetworkGenerator`.  No external tools
            required.

        When an explicit ``'pysb'`` or ``'bng'`` is requested but
        equations generated by the *other* generator already exist, a
        :class:`UserWarning` is emitted and the existing equations are
        reused.  Call :meth:`~pysb.core.Model.reset_equations` first to
        force regeneration.  The generator that produced the current
        equations is recorded on the model as ``model._netgen_source``
        (``'pysb'`` or ``'bng'``).
    ...
    netgen : str
        Network generator as passed by the caller (``'auto'``, ``'bng'``,
        or ``'pysb'``).  The generator actually used is recorded on the
        model as ``model._netgen_source`` after the first simulation run.
    ...
    def __init__(self, model, tspan=None, initials=None,
        ...
        self.tout = None
        # Network generator selection
        _netgen = kwargs.pop('netgen', 'auto')
        if _netgen not in ('auto', 'bng', 'pysb'):
            raise ValueError(
                "netgen must be 'auto', 'bng', or 'pysb', got {!r}".format(
                    _netgen)
            )
        if _netgen == 'bng' and model.auto_netgen:
            raise ValueError(
                "netgen='bng' is incompatible with a model built with "
                "auto_netgen=True. To use BioNetGen, first call "
                "model.reset_equations() and set model.auto_netgen = False."
            )
        self.netgen = _netgen
    ...
    def _run_netgen(self, cleanup=True):
        """Generate equations using the configured network generator.

        Respects ``self.netgen`` ('auto', 'bng', or 'pysb').  See the
        class docstring for the full resolution rules.

        Parameters
        ----------
        cleanup : bool, optional
            Passed to :func:`pysb.bng.generate_equations` when using BNG.
            Ignored for the native generator.
        """
        model = self._model
        requested = self.netgen

        if requested == 'auto':
            # Use pre-existing equations if any, regardless of source.
            if model._reactions:
                return
            effective = 'pysb' if model.auto_netgen else 'bng'
        else:
            effective = requested
            # If equations from a different generator already exist, warn
            # and reuse them rather than silently regenerating.
            source = getattr(model, '_netgen_source', None)
            if model._reactions and source is not None and source != effective:
                warn(
                    "Equations were already generated by {!r} but "
                    "netgen={!r} was requested; reusing existing equations. "
                    "Call model.reset_equations() first to regenerate with "
                    "{!r}.".format(source, effective, effective),
                    UserWarning,
                    stacklevel=3,
                )
                return
        ...
        if effective == 'pysb':
            if not model._reactions:
                from pysb.netgen import NetworkGenerator
                ng = NetworkGenerator(model)
                ng.generate_network()  # populate=True (default)
                model._netgen_dirty = False
                model._netgen_source = 'pysb'
        else:
            # auto_netgen is guaranteed False here (ValueError in __init__
            # rejects the netgen='bng' + auto_netgen=True combination).
            import pysb.bng
            pysb.bng.generate_equations(
                model, cleanup=cleanup, verbose=self.verbose
            )
