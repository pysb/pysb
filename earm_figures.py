...
def fig_4b():
    ...
    #x_norm = array([x[s] for s in obs]).T   # alternate DRY indexing approach
    x_norm = 1 - x_norm / x_norm[0, :]  # gets away without max() since first values are largest

    # this is what I originally thought 4B was plotting
    #x_norm = array([x['tBid'], x['CPARP'], x['cSmac']]).T
    #x_norm /= x_norm.max(0)

    p = plot(tp, x_norm[:,0], 'b', tp, x_norm[:,1], 'y', tp, x_norm[:,2], 'r')
    figlegend(p, obs, 'upper left')
    a = gca()
    a.set_ylim((-.05, 1.05))
    show()
